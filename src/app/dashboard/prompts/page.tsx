...
const PROMPT_TARGET_ENGINES = [
  ...
  { id: 'claude', label: 'Claude 3.5 Sonnet', icon: '🟠', badge: 'Anthropic' },
  { id: 'copilot', label: 'MS Copilot', icon: '🔷', badge: 'Bing' },
  { id: 'meta', label: 'Meta AI', icon: '🟦', badge: 'Llama 3' },
];
...
export default function PromptsPage() {
  ...
  const handleSelectAllEngines = () => {
    if (selectedEngineIds.length === PROMPT_TARGET_ENGINES.length) {
      setSelectedEngineIds(['gemini']);
    } else {
      setSelectedEngineIds(PROMPT_TARGET_ENGINES.map((e) => e.id));
    }
  };
  ...
  const handleAddQuery = async (e: React.FormEvent) => {
    ...
    const formattedEngine =
      selectedEngineIds.length === PROMPT_TARGET_ENGINES.length
        ? 'All 6 AI Engines'
        : PROMPT_TARGET_ENGINES.filter((e) => selectedEngineIds.includes(e.id))
            .map((e) => e.label)
            .join(', ');
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="sm:col-span-2">
                  <label className="block text-xs font-semibold text-slate-700 mb-1.5">
                    Target Model Architecture (Select Primary Engine)
                  </label>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {[
                      { id: 'gemini-1.5-flash', label: 'Gemini 1.5 Flash', badge: 'Fast Extraction', icon: '🟣' },
                      { id: 'gemini-3.7-flash', label: 'Gemini 3.7 Flash', badge: 'High Reasoning', icon: '🟣' },
                      { id: 'gpt-4o', label: 'OpenAI GPT-4o', badge: 'Search & Canvas', icon: '🟢' },
                      { id: 'sonar-pro', label: 'Perplexity Sonar', badge: 'Web Grounded', icon: '🔵' },
                    ].map((mod) => (
                      <label
                        key={mod.id}
                        className={`flex items-center gap-2 p-2 rounded-xl border text-xs cursor-pointer select-none transition-all ${
                          editModelTarget === mod.id
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={editModelTarget === mod.id}
                          onChange={() => setEditModelTarget(mod.id)}
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-[9px] text-slate-400 font-normal truncate">{mod.badge}</div>
                        </div>
                      </label>
                    ))}
                  </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in overflow-y-auto">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-2xl max-w-xl w-full animate-in zoom-in-95 my-8 max-h-[92vh] overflow-y-auto space-y-4">
            ...
            <form onSubmit={handleAddQuery} className="space-y-4">
              ...
              {/* AI Target Engine Checkboxes */}
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-[11px] font-semibold text-slate-700">
                    Target AI Search Engines ({selectedEngineIds.length}/{PROMPT_TARGET_ENGINES.length} active)
                  </label>
                  <button
                    type="button"
                    onClick={handleSelectAllEngines}
                    className="text-[11px] font-semibold text-indigo-600 hover:text-indigo-800 cursor-pointer"
                  >
                    {selectedEngineIds.length === PROMPT_TARGET_ENGINES.length ? 'Deselect All' : 'Select All Engines'}
                  </button>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {PROMPT_TARGET_ENGINES.map((eng) => {
                    const isChecked = selectedEngineIds.includes(eng.id);
                    return (
                      <label
                        key={eng.id}
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={isChecked}
                          onChange={() => toggleEngineSelection(eng.id)}
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="text-xs truncate leading-tight">{eng.label}</div>
                          <div className="text-[10px] text-slate-400 font-normal">{eng.badge}</div>
                        </div>
                      </label>
                    );
                  })}
